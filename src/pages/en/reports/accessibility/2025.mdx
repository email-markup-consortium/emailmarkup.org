...
We have spent the last year collecting emails as part of our [data collection project](/en/projects/#data-collection). Since the last report, we’ve collected 443,870 emails to be included in this years accessibility report, about 23,000 more email than were analyzed in the 2024 report.

Out of that number, 285 emails could not be analyzed, bringing the total of emails analysed down to 443,585. Only 21 emails contained no issues, representing two brands.
...
We were hoping to see better standards across education and government emails because generally there are stricter accessibility laws for these. Although both groups have a 100% failure rate, the Critical issues are much lower than the overall stats 4% and 7% respectively. But the sample data is low, combined it's less than 1% of total message, so we can't make strong judgements here.

Although we have seen some improvements, there is still a long way to go. At least 99.89% of email we looked at are broken, probably a lot more if we were to include manual testing. Broken emails should be the exception, not the standard.

Accessibility is everyones responsibility. Every email should go through a proper QA process before it's sent. Not everyone will have the skills to fix accessibility issues — but everyone should be able to find them and report them so they can be addressed.

Senders should be giving accessibility issues the same priority as rendering bugs. It's not uncommon for an email to be delayed until it looks right in Outlook — accessibility deserves that same level of attention.

Email creation tools need to do better by improving default templates, setting accessible defaults for components, and offering fallbacks or warnings when users create inaccessible content.

Email clients need to ensure they support [standard accessible code](/en/reports/email-clients/feature-support-rankings/). And any unsupported code is handled in a way that will fallback gracefully, less likely the create accessibility issues.
