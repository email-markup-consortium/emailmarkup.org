...
To learn more about and help expand our dataset of emails, visit the [Data Collection Project](/en/projects/#data-collection).
...
It is also worth mentioning that passing automated testing does not guarantee that an email is accessible. Email that pass our checks may still have accessibility issues that we cannot pick up through automated testing. For example, we check if an alt attribute is present on an `<img>` tag, but we do not check if the text is suitable for that image in the context of that message. 

Something like a `<title>` element is not required for an email to be accessible, however a large number of emails have a view in browser version that shows the same content. In this context `<title>` is required. If there is no browser version, or if the browser version has a `<title>` inserted into it, then this rule doesn't matter and the email would pass manual testing.
...
One underused opportunity could be building tools and frameworks flagging warnings for accessibility issues in real time rather than asking users to run checks. As a user selects a color, run a contrast check. As a user adds an image, help them with alt text. As they add a heading, make sure it's not skipping levels.
...
No email client in this round supported all **37** accessibility features. The highest score was **34/37** (Apple Mail on macOS). However, those 3 failing tests all have partial support. Samsung with a score of 31/37 only has 2 failing tests, one point is lost for a partially supported test and 3 are untested. Also credit to ProtonMail with a score of 29/37 we know the team there have been doing a lot of good work and seeing some big improvements in the results here.
...
| 29 | ProtonMail (Desktop webmail) |
| 29 | ProtonMail (iOS) |
| 26 | ProtonMail (Android) |
| 25 | Outlook (Android) |
...
For some of the newer CSS features like `light-dark()`, it's understandable that email clients may be a bit behind web. However, for features like `:hover`, `:focus` that have been in the CSS specification since 1998 and are supported by all modern browsers, it's very disappointing to not support them.
