...
We are seeing some small improvements in the 2026 accessibility report, however too slow. We're still seeing 99.88% of email fail basic testing. 
...
Issue present in 15.08% (56,753) of emails tested.

Number of appearances: 56,753.
...
99.13% had tables used for formatting that should have the `role` attribute set to `presentation` or `none`.

Also, 97.39% were missing an `<h1>` element.
...
Although it's only a mild issue **55.7%** of emails had Duplicate ID's
...
We were hoping to see better standards across education and government emails because generally there are stricter accessibility laws for these. Although both groups have a 100% failure rate, the Critical issues are much lower than the overall stats, 2% and 7% respectively. However the sample data size for these two sectors is small: combined it's less than 1% of total messages analyzed.
