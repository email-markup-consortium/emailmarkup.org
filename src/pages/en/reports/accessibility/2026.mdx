...
This year's report we are also looking beyond the sender and at widely-used sender platforms with built-in email build/authoring tools. We audited 10,566 emails sent via Substack, Shopify and Beehiiv. None passed our automated testing.

When it comes to email clients, we have taken a deeper look this year, nearly doubling our email client accessibility criteria from 20 to [37 feature benchmarks](/en/reports/email-clients/feature-support-rankings/). The data is clear: a major reason why accessibility is failing in email is a lack of implementation.

This report serves as a technical audit of the senders, platforms and email clients that define the modern inbox. It highlights the responsibility is divided by evaluating the systemic barriers within email clients and marketing platforms instead of solely pointing at the senders.
...
And to scale these results, high-performing teams are moving toward accessible design systems. By enforcing accessibility into a single source of truth or a shared component library, fixes are implemented once and propagated globally. This eliminates the "human error" every individual campaign and allows for meaningful iterations and improvements over time.
...
Email that pass our checks may still have accessibility issues that we cannot pick up through automated testing. 

After manually testing the passing emails we found both Customer.io and Naomi West has small issues with alt text not matching text in images well enough. 

For Noble Panacea found a few issues:
