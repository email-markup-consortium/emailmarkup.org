...
This year's report we are also looking beyond the sender and at widely-used newsletter platforms with built-in email build/authoring tools. We audited 10,566 emails sent via Substack, Shopify and Beehiiv. None passed our automated testing.
...
This year we nearly doubled our email client accessibility criteria from 20 to [37 feature benchmarks](/en/reports/email-clients/feature-support-rankings/). No client supports all of them, but we recognize the progress from leaders like Apple Mail (34/37 on macOS), Samsung Email (31/37), and Proton Mail (29/37).
...
In alignment with our commitment to transparency and collaboration, the broader trends of our data can be found on the [Email Markup Database](https://database.emailmarkup.org/), which serves as a comprehensive, continuosly updated repository of real-world HTML and CSS usage metrics in email.
...
We detected 6,909 Substack emails, 1,468 Shopify emails, and 2,189 Beehiiv emails. These have only been tracked for the last six months, however emails from these senders may still appear in the full twelve months of data used elsewhere in this report.
 ...
 * Design and code alignment: Checking semantic markup again design and content.
...
Senders cannot fully work around email clients limitations and produce fully accessible HTML messages. When an email client intentionally strips or alters semantic markup it has a clear responsibility to document these breaking changes. The data in the next section backs this.
...
#### HTML features:
  ...
  1. `role` attribute
  1. `<ruby>` element
  1. HTML5 semantics (`<article>`, `<aside>`, `<details>`, `<figcaption>`, `<figure>`, `<footer>`, `<header>`, `<main>`, `<mark>`, `<nav>`, `<section>`, `<summary>`, `<time>`)
  1. `<table>` element
...
1. Samsung Email is second with a score of 31/37 only has 2 failing tests, one point is lost for a partially supported test and 3 are untested.
1.  ProtonMail  scored  29/37. We are in communication with the team and know they have been doing a lot of good work and allocating regular resource for this. 
...
| 29 | ProtonMail (Desktop webmail), ProtonMail (iOS) |
...
For a company that promotes an accessible web, that gap remains a notable inconsistency. Google has been listed in 2026's [Forbes Accessibility 200](https://www.forbes.com/lists/accessibility/). It is recognized globally for its good accessibility work. This makes it really difficult to reconcile their excellent accessibility work in the digital space with the limited attention to accessibility support in Gmail, one of their main products.
...
We believe email clients, ESPs, build tools and senders can work together toward an open, reliable, and inclusive email ecosystem. This ultimately benefits the end users (recpients) the most. In order to get there, all parties must understand their role and responsibility in this unique ecosystem.  
